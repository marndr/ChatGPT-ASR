
    ## Test 0
    ASR transcription:            I prefer see over coffee.
    Corrected ASR transcription:  I prefer tea over coffee.
    Reference transcription:      I prefer tea over coffee.
    CER: 0.00%    CER(No chatgpt): 8.33%%
    WER: 0.00%    WER(No chatgpt): 20.00
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 1
    ASR transcription:            I need to catch a drain to London.
    Corrected ASR transcription:  I need to catch a train to London.
    Reference transcription:      I need to catch a train to London.
    CER: 0.00%    CER(No chatgpt): 3.03%%
    WER: 0.00%    WER(No chatgpt): 12.50
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 2
    ASR transcription:            I'll beat you at the coffee shop.
    Corrected ASR transcription:  I'll meet you at the coffee shop.
    Reference transcription:      I'll meet you at the coffee shop.
    CER: 0.00%    CER(No chatgpt): 6.25%%
    WER: 0.00%    WER(No chatgpt): 14.29
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 3
    ASR transcription:            He's a professional sheaf.
    Corrected ASR transcription:  He's a professional chef.
    Reference transcription:      He's a professional chef.
    CER: 0.00%    CER(No chatgpt): 8.00%%
    WER: 0.00%    WER(No chatgpt): 25.00
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 4
    ASR transcription:            The leather is nice today.
    Corrected ASR transcription:  The weather is nice today.
    Reference transcription:      The weather is nice today.
    CER: 0.00%    CER(No chatgpt): 4.00%%
    WER: 0.00%    WER(No chatgpt): 20.00
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 5
    ASR transcription:            Please pass me the vault.
    Corrected ASR transcription:  Please pass me the bolt.
    Reference transcription:      Please pass me the salt.
    CER: 8.70%    CER(No chatgpt): 8.33%%
    WER: 20.00%    WER(No chatgpt): 20.00
    SER: 100.00%    SER(No chatgpt): 100.00
    ---

    ## Test 6
    ASR transcription:            I'll be their in five minuets.
    Corrected ASR transcription:  I'll be there in five minutes.
    Reference transcription:      I'll be there in five minutes.
    CER: 0.00%    CER(No chatgpt): 13.79%%
    WER: 0.00%    WER(No chatgpt): 33.33
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 7
    ASR transcription:            I'm going to the gross restore.
    Corrected ASR transcription:  I'm going to the grocery store.
    Reference transcription:      I'm going to the grocery store.
    CER: 0.00%    CER(No chatgpt): 16.67%%
    WER: 0.00%    WER(No chatgpt): 33.33
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 8
    ASR transcription:            I won't to go two the beech.
    Corrected ASR transcription:  I want to go to the beach.
    Reference transcription:      I want to go to the beach.
    CER: 0.00%    CER(No chatgpt): 14.81%%
    WER: 0.00%    WER(No chatgpt): 42.86
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 9
    ASR transcription:            The son is shining brightly in the sky.
    Corrected ASR transcription:  The sun is shining brightly in the sky.
    Reference transcription:      The sun is shining brightly in the sky.
    CER: 0.00%    CER(No chatgpt): 2.63%%
    WER: 0.00%    WER(No chatgpt): 12.50
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    ## Test 10
    ASR transcription:            The son sets at the beech are always stunting.
    Corrected ASR transcription:  The sun sets at the beach are always stunning.
    Reference transcription:      The sun sets at the beach are always stunning.
    CER: 0.00%    CER(No chatgpt): 6.67%%
    WER: 0.00%    WER(No chatgpt): 33.33
    SER: 0.00%    SER(No chatgpt): 100.00
    ---

    CER_avg: 0.79%    CER_original_avg: 8.41%%
    WER_avg: 1.82%    WER_original_avg: 24.29
    SER_avg: 9.09%    SER_original_avg: 100.00
    ---
