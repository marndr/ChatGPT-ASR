...
for i, (audio_path, reference_transcription) in enumerate(librispeech_data.items()):
    ...
    messages = [
        ...
        keys: response and probability. response is the corrected_ASR_output and probability shows the likelihood of each correction. \
    ...
    SER = jiwer_mer(corrected_asr_transcription , reference_transcription) * 100
    WER = jiwer_wer(corrected_asr_transcription , reference_transcription) * 100


    # calculate CER, SER, and WER using jiwer for original transcription and reference.
    ...
    SER_original = jiwer_mer(asr_transcription , reference_transcription) * 100
